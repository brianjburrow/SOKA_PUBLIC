...
class ActivityViewTestCase(TestCase):
    ...
    def test_individual_activity(self):
        with self.client as c:
            ...
            # You cannot view athlete's activities unless you are following them
            resp = c.get(f'/activities/{self.test_activity.id}')
            self.assertEqual(resp.status_code, 302)

            # You cannot view athlete's activities unless you are following them
            resp = c.get(f'/activities/{self.test_activity.id}', follow_redirects = True)
            self.assertEqual(resp.status_code, 200)
            html = resp.get_data(as_text=True)
            self.assertIn("You must be following an athlete to view their activities", html)

            # Delete should fail, because you can only delete your own activities
            resp = c.post(f'/activities/{self.test_activity.id}/delete')
            self.assertEqual(resp.status_code, 302)

            # Deletesshould fail, because you can only delete your own activities
            # Test to see if the response is correct after redirecting
            resp = c.post(f'/activities/{self.test_activity.id}/delete', data = {}, follow_redirects = True)
            self.assertEqual(resp.status_code, 200)
            ...
            # Admin can access activities without following user
            resp = c.get(f'/activities/{self.test_activity.id}')
            self.assertEqual(resp.status_code, 200)

            # Admin can delete activities
            resp = c.post(f'/activities/{self.test_activity.id}/delete', follow_redirects = True)
            self.assertEqual(resp.status_code, 200)
            html = resp.get_data(as_text=True)
            self.assertIn("Activity deleted successfully", html)


            # Change the session to the activity owner
            with c.session_transaction() as sess:
                sess[CURR_USER_KEY] = self.non_admin.id
            # select a new activity as the test activity
            self.test_activity = Activity.query.filter(Activity.user_id == self.non_admin.id).first()
            resp = c.get(f'/activities/{self.test_activity.id}')
            self.assertEqual(resp.status_code, 200)
    ...
    def test_new_activity(self):
        ...
        with self.client as c:
            # Cannot get the new activity form without logging it
            resp = c.get('/activities/new')
            self.assertEqual(resp.status_code, 302)

            # Cannot get the new activity form without logging it
            resp = c.get('/activities/new', follow_redirects = True)
            self.assertEqual(resp.status_code, 200)
            html = resp.get_data(as_text = True)
            self.assertIn("You must be logged in to upload an activity", html)

            # Cannot make a post request without logging in
            data = {}
            resp = c.post('/activities/new', data = data, follow_redirects=True)
            html = resp.get_data(as_text = True)
            self.assertIn("You must be logged in to upload an activity", html)
            ...
            resp = c.get('/activities/new')
            self.assertEqual(resp.status_code, 200)
            resp = c.post('/activities/new', data = data, content_type='multipart/form-data')
            self.assertEqual(resp.status_code, 302)
            pass
